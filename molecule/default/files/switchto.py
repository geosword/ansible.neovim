...
def getActiveWID():
	...
	# Some more on this: So sometimes, the Wid is 0x00[1-9] and presumably COULD be 0x000 or 0x0000!
	# so doing this regex only solves a single case....
	# get rid of the leading '0x'
	widonly = re.sub(r'^0x','',line[4])
	# now zfill to get our leading 0's
	widonly = widonly.zfill(8)
	# now return with '0x' prepended
	return '0x' + widonly
...
def getDesktops():
    swlog("getting the current desktop")
    stdout, stderr = wmctrl(["-d"])
    stdout = stringDecode(stdout)
    fields = ['did','active','resolution1','vp','vpnum','wa','wanum','resolution2','desktop','desktopname']
    desktops = []
    activedesktop = 0
    for line in stdout.splitlines():
        desktop = dict(zip(fields, line.split()))
        # If the desktop ID is -1 then its not actually a window, but a screen (or a plasma "start" bar)
        desktops.append(desktop)
        if desktop['active'] == '*':
            activedesktop = desktop['did']
    return desktops, activedesktop

def getByClass(windows, wclass, preferdid=-2):
    swlog("getByClass: finding windows by class [" + wclass + "]")
    matches = []
    wids = {}
    for i in windows:
        # explaination for this monstrosity: if preferdid is set to something other
        # than -2 (which means I dont care what desktop the window is on) then
        # just match on the windowclass. if its set to something 0 or over, then assume
        # I want to match windows on THAT desktop id ONLY... OR windows that appear
        # on ALL desktops (which will have a did of -1)
        if (int(preferdid) == -2 and i["class"].lower() == wclass.lower()) or \
           (int(preferdid) >= 0 and i["class"].lower() == wclass.lower() and \
            (int(i["did"]) == int(preferdid) or int(i["did"]) == -1) ):
...
def writeClassWids(wclass, wids, did, path):
    swlog("writeClassWids: Writing wids to " + path + "/" + wclass + "-" + did + ".tmp")
    with open(path + "/" + wclass.lower() + "-" + did + ".tmp", "w") as f:
        for item in wids:
            f.write("%s\n" % item)
...
def loadClassWids(wclass, did, path):
    swlog("loadClassWids: Loading wids from " + path + "/" + wclass + "-" + did + ".tmp")
    with open(path + "/" + wclass.lower() + "-" + did + ".tmp") as f:
...
desktops, activedesktop = getDesktops()
swlog('Active desktop is ' + activedesktop)

activeWid = getActiveWID()
matchingWindows, mwidx = getByClass(windows, args.wclass, activedesktop)
...
if len(matchingWindows) == 0:
	...
	if len(matchingWindows)>1:
		...
		if os.path.isfile(args.tempdir + '/' + args.wclass.lower() + '-' + activedesktop + '.tmp'):
			# load the wids into a list
			loadedMatchingWindows = loadClassWids(args.wclass,activedesktop,args.tempdir)
			# then remove any wids we loaded, that no longer exist from the list / might have been created since the last run
			loadedMatchingWindows,mwidx = reconcileWids(loadedMatchingWindows, matchingWindows)
			# If none of the wids in the tempfile are still current, we'll just carry on using the search results we got earlier
			swlog('This is the list after reconcilliation: ' + str(loadedMatchingWindows))
		...
		swlog('widx is [' + ' '.join(widx) + ']')
		if windows[widx[activeWid]]['class']==args.wclass:
			swlog('switching to window with the same wclass [' + args.wclass + '] so cycling the head')
			# remove the first match from the head of the list
			swlog('popping this off the head of the list [' + matchingWindows[mwidx[activeWid]] + ']')
			matchingWindows.pop(mwidx[activeWid])
			# Now add it to the end of the list
			matchingWindows.append(activeWid)
		# write the wids back to the tempfile
		writeClassWids(args.wclass,matchingWindows,activedesktop,args.tempdir)
